...
int main()
{
  ...
  if (signal == 0) {
    ...
    PhysicalNumber e(80, Unit::SEC);
    PhysicalNumber f(200, Unit::G);
    PhysicalNumber g(1, Unit::TON);
    PhysicalNumber h(10, Unit::MIN);
    PhysicalNumber i(10, Unit::MIN);
    ...
    .setname("MyTest")
    .CHECK_OK(istringstream("60[sec]") >> e)
    .CHECK_OK(istringstream("20[min]") >> h)
    .CHECK_OUTPUT(e, "60[sec]")
    .CHECK_OUTPUT(h, "20[min]")
    .CHECK_OUTPUT(d+h-h, "30[min]") //Checks if the output is right.
    .CHECK_OUTPUT(g, "1[ton]")
    .CHECK_OUTPUT(f, "200[g]")
    .CHECK_OUTPUT(g+f, "1000200[g]") //Checks if it converts the TON to Gram and sums them.
    .CHECK_OUTPUT(-(g+f), "-1000200[g]") //Checks if the negative onary operator works(after sum).
    .CHECK_OUTPUT(+(g+f), "1000200[g]") //Checks if the positive onary operator works(after sum).
    .CHECK_OUTPUT(b, "300[m]")
    ...
    .CHECK_THROWS(d+g) //Checks if MINUTS + TON throws exception.   
    .CHECK_THROWS((h+=g)) //Checks if MINUTE += TON throws exception.
    .CHECK_THROWS((h-=g)) //Checks if MINUTE -= TON throws exception.
    .CHECK_OUTPUT((d>h), "true")
    .CHECK_OUTPUT((h<d), "true")
    .CHECK_OUTPUT((c<=d), "false")
    .CHECK_OUTPUT((h+=i), "30[min]")
    .CHECK_THROWS((d+=g)) //Checks if MINUTS += TON throws exception.
    .CHECK_OUTPUT(h, "30[min]") //Checks if the change worked well.
    .CHECK_OUTPUT(d, "30[min]")
    .CHECK_OUTPUT((h==d),"true")
    .CHECK_OUTPUT(c, "2[hour]")
    .CHECK_OK(istringstream("3[hour]") >> c)
    .CHECK_OUTPUT(c, "3[hour]") //Checks if the change worked well.
    .CHECK_THROWS((c-=a)) //Checks if HOUR -= KILO GRAM throws exception.
    .CHECK_THROWS((c+=a)) //Checks if HOUR += KILO GRAM throws exception.
    .CHECK_THROWS((d-g)) //Checks if MINUTS -= TON throws exception.
    .CHECK_OUTPUT(g, "1[ton]") 
